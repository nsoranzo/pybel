...
class TestGraphCache(BelReconstitutionMixin, unittest.TestCase):
    ...
    def test_load_reload(self, mock_get):
        ...
        self.gcm.insert_graph(self.g, store_parts=False)
    ...
    def test_get_versions(self, mock_get):
        ...
        self.g.document[METADATA_VERSION] = TEST_V1
        self.gcm.insert_graph(self.g, store_parts=True)

        self.g.document[METADATA_VERSION] = TEST_V2
        self.gcm.insert_graph(self.g, store_parts=True)

        self.assertEqual({TEST_V1, TEST_V2}, set(self.gcm.get_graph_versions(self.g.document[METADATA_NAME])))

        g_recent = self.gcm.get_graph(self.g.document[METADATA_NAME])
        self.assertEqual(TEST_V2, g_recent.document[METADATA_VERSION])

    @mock_bel_resources
    def test_get_or_create_node(self, mock_get):
        network = self.gcm.insert_graph(self.g, store_parts=True)
